---
layout: page
title: Certifications & Professional Development
---

## Professional Certifications

All certifications have been completed and verified through their respective issuing organizations.

---

## 1. Human-Centered Design for Inclusive Innovation
**Issuer:** University of Toronto (via Coursera)  
**Date Completed:** 2023  
**Specialization:** Gender Analytics - Gender Equity through Inclusive Design

<img src="{{ "/img/projects/innovation.png" | prepend: site.baseurl }}" style="width: 100%; height: auto; max-height: 250px; object-fit: cover; border-radius: 8px; margin: 20px 0;">

### About This Certification
This comprehensive online course was offered through the Institute for Gender and the Economy (GATE) at the University of Toronto's Rotman School of Management. It provides advanced training in human-centered design methodologies with a focus on creating inclusive solutions for diverse populations.

**Course Instructors:**
- **Sarah Kaplan** - Distinguished Professor; Director, Institute for Gender and the Economy (GATE), Rotman School of Management
- **Mark Leung** - Former Director, Business Design Lab, Rotman School of Management
- **Nika Stelman** - Director, Design Strategy, Bridgeable

**Key Learning Areas:**
- Inclusive design principles and frameworks
- Gender equity in organizational contexts
- Stakeholder engagement and research methods
- Design thinking applied to social innovation
- Gender analytics and diversity metrics

**Credential Link:** [https://coursera.org/share/75c6c86ad5eb7344d608e594b3fe7044](https://coursera.org/share/75c6c86ad5eb7344d608e594b3fe7044)

---

## 2. Human Resources: Using Metrics to Drive HR Strategy
**Issuer:** LinkedIn Learning  
**Date Completed:** September 22, 2022  
**Recertification Credits:** 1.25 hours (HRCI)

<img src="{{ "/img/projects/hr-metrics.png" | prepend: site.baseurl }}" style="width: 100%; height: auto; max-height: 250px; object-fit: cover; border-radius: 8px; margin: 20px 0;">

### About This Certification
Certified through the HR Certification Institute (HRCI), this course focuses on leveraging data analytics and metrics to make strategic human resource decisions. The training emphasizes alignment between HR metrics and organizational performance.

**HRCI Recertification Details:**
- **Provider ID:** 604152
- **Certificate Number:** AazvpB-itQtGQ_CQBvojLlpSRygB
- **Activity Number:** 575392
- **Program:** HR Certification Institute® (HRCI®)

**Key Topics Covered:**
- HR metrics and KPIs
- Data visualization for HR insights
- Strategic workforce planning
- Performance analytics
- Talent management metrics
- ROI calculations for HR initiatives

---

## 3. Learn Database Design With MySQL: Introduction
**Issuer:** upGrad  
**Date Completed:** September 23, 2022  
**Co-signer:** Mayank Kumar, Co-founder & MD

<img src="{{ "/img/projects/mysql.png" | prepend: site.baseurl }}" style="width: 100%; height: auto; max-height: 250px; object-fit: cover; border-radius: 8px; margin: 20px 0;">

### About This Certification
A foundational certification in relational database design and SQL fundamentals. This course provides essential knowledge in database architecture, data modeling, and MySQL implementation.

**Key Topics Covered:**
- Relational database concepts
- Database design principles
- Entity-relationship modeling (ER diagrams)
- MySQL fundamentals
- SQL queries and operations
- Data normalization
- Database optimization basics

---

## Professional Development Summary

These certifications represent a strategic focus on three key areas:

1. **Inclusive Design & Gender Analytics** - Ensuring organizational systems and processes serve diverse populations equitably
2. **HR Strategy & Metrics** - Using data-driven approaches to optimize human capital management
3. **Data Management** - Building foundational knowledge in database systems for operational excellence

Together, they support my professional mission to design efficient, inclusive, and data-driven organizations.

---

## Continuous Learning

I remain committed to professional development and regularly pursue training opportunities in:
- Advanced financial analytics and modeling
- International HR practices
- Emerging technologies in business operations
- Gender equity and inclusive innovation

*For verification of any certification, please contact me directly.*
